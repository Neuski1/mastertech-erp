...
export const api = {
  ...
  markLeadNotSpam: (id) => request(`/leads/${id}/not-spam`, { method: 'POST' }),
  ...
  getReviewRequests: () => request('/communications/review-requests'),
  ...
  // Rate increase works from a TARGET per-foot rate and a chosen set of boxes,
  // so a customer already signed at the new pricing is never raised twice.
  previewStorageRateIncrease: (data) => request('/storage/rate-increase/preview', { method: 'POST', body: JSON.stringify(data) }),
  applyStorageRateIncrease: (data) => request('/storage/rate-increase/apply', { method: 'POST', body: JSON.stringify(data) }),
  sendStorageRateNotices: (data) => request('/storage/rate-increase/notices', { method: 'POST', body: JSON.stringify(data) }),
  getStorageRateNoticesPending: () => request('/storage/rate-increase/pending'),
  ...
  updateSupplierFields: (id, data) => request(`/suppliers/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  recomputeSupplierLeadTimes: () => request('/suppliers/recompute-lead-times', { method: 'POST' }),
  ...
  getBankReconciliation: (asOf) => request(`/bookkeeping/reports/bank-reconciliation${asOf ? `?as_of=${asOf}` : ''}`),
