...
export default function RecordList() {
    ...
    <tr key={r.id} onClick={() => navigate(`/records/${r.id}`)} style={{ cursor: 'pointer' }}>
      ...
      {canSeeFinancials && (
        <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 600 }}>
          {formatCurrency(showPaidDate ? r.total_sales : r.amount_due)}
        </td>
      )}
    </tr>
  );
  ...
  const sortGroupRecords = (recs, groupKey) => {
    ...
    return [...recs].sort((a, b) => {
      ...
      } else if (field === 'record_number' || field === 'amount_due' || field === 'total_sales') {
        va = parseFloat(va) || 0;
        vb = parseFloat(vb) || 0;
      } else {
  ...
  const renderMobileCard = (r, showPaidDate = false) => (
    <div key={r.id} className="mobile-record-card" onClick={() => navigate(`/records/${r.id}`)}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.8rem', color: '#9ca3af' }}>
        ...
        {canSeeFinancials && <span style={{ fontWeight: 600, color: '#374151' }}>{formatCurrency(showPaidDate ? r.total_sales : r.amount_due)}</span>}
      </div>
    </div>
  );
    ...
    return (
      <thead>
        <tr>
          ...
          {canSeeFinancials && (
            showPaidDate
              ? <th style={{ ...thStyle, ...sortable, textAlign: 'right' }} onClick={onClick('total_sales')}>Amount Paid{sortArrow(groupKey, 'total_sales')}</th>
